...
window.addEventListener("DOMContentLoaded", function () {
    ...
    // if we can not use any of HTML5 based camera ways then we use Flash based camera
    {
        ...
        document.getElementById("flashOut").innerHTML = (webcam.get_html(640, 400));
    }
...
function UploadToCloud() {
    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            if (data.d.length != 0) {
                ...
                //if (htmlSelect.value = '*DEMO*') {
                //    htmlSelect.value= text.replace(htmlSelect.value, "");
                //    timer = setTimeout(UploadToCloud, 3000);
                //    alert(htmlSelect.value);
                //}
    ...
    var sbarcode = document.getElementById('OutListBoxHTML5').value;
...
function take_snapshot() {
    ...
    webcam.set_api_url('/Dashboard/UploadFlash?type=' + selection);
